<div align="center">

# Hey, I'm Dakshanya Maddala 👋

### Machine Learning Engineer • Data Scientist • AI Researcher

Building intelligent systems at the intersection of
**Artificial Intelligence, Robotics, Computer Vision and Large-Scale Data**

<br>

<a href="mailto:maddaladakshanya@gmail.com">
  <img src="https://img.shields.io/badge/Outlook-Contact_Me-0078D4?style=for-the-badge&logo=microsoftoutlook&logoColor=white">
</a>
<a href="https://www.linkedin.com/in/dakshanya-maddala/">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>
<a href="https://github.com/Duck-exe">
  <img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
</a>

</div>

<!-- <p align="center">
  <img src="https://komarev.com/ghpvc/?username=Duck-exe&label=Profile+Views&color=0e75b6&style=for-the-badge"/>
</p> -->

---

## 👨‍💻 About Me

* 🎓 Ph.D. researcher in **Electrical Engineering and Computer Science**
* 🤖 Former Data Scientist at **Amazon Robotics**
* 🧠 Interested in **Machine Learning, Artificial Intelligence and Data Science**
* 🕵️ Building systems involving **AI agents, LLMs and retrieval-augmented generation**
* 👁️ Exploring **computer vision, robotics and embodied intelligence**
* 📊 Experienced in analyzing large-scale telemetry, spatial and time-series data
* 🔬 Researching optimization, wireless sensor networks and multi-agent systems
* 📚 Published research in IEEE conferences and journals
* 🌱 Currently strengthening my skills in production ML, agentic AI and scalable inference
* 💬 Ask me about Python, machine learning, robotics, LLMs, RAG or research

---

## 🧠 Areas of Interest

<div align="center">

![Machine Learning](https://img.shields.io/badge/Machine_Learning-102230?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![Artificial Intelligence](https://img.shields.io/badge/Artificial_Intelligence-412991?style=for-the-badge)
![Data Science](https://img.shields.io/badge/Data_Science-150458?style=for-the-badge&logo=jupyter&logoColor=white)
![AI Agents](https://img.shields.io/badge/AI_Agents-000000?style=for-the-badge&logo=openai&logoColor=white)
![Large Language Models](https://img.shields.io/badge/LLMs-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)

<br>

![RAG](https://img.shields.io/badge/RAG-00A86B?style=for-the-badge)
![Computer Vision](https://img.shields.io/badge/Computer_Vision-5C3EE8?style=for-the-badge)
![Robotics](https://img.shields.io/badge/Robotics-22314E?style=for-the-badge&logo=ros&logoColor=white)
![Distributed Systems](https://img.shields.io/badge/Distributed_Systems-00599C?style=for-the-badge)
![Generative AI](https://img.shields.io/badge/Generative_AI-FF6F61?style=for-the-badge)

</div>

---

## 🛠️ Technical Skills

### Programming Languages

<p>
  <img src="https://skillicons.dev/icons?i=python,cpp,java,bash" alt="Programming languages">
  <img src="https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white" height="48" alt="SQL">
</p>

### Machine Learning and Data Science

<p>
  <img src="https://skillicons.dev/icons?i=pytorch,sklearn" alt="Machine learning tools">
  <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white" alt="NumPy">
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white" alt="Pandas">
  <img src="https://img.shields.io/badge/Hugging_Face-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black" alt="Hugging Face">
</p>

### AI and Computer Vision

<p>
  <img src="https://img.shields.io/badge/LLMs-412991?style=for-the-badge" alt="LLMs">
  <img src="https://img.shields.io/badge/RAG-000000?style=for-the-badge" alt="RAG">
  <img src="https://img.shields.io/badge/AI_Agents-10A37F?style=for-the-badge&logo=openai&logoColor=white" alt="AI Agents">
  <img src="https://img.shields.io/badge/CNNs-FF6F00?style=for-the-badge" alt="CNNs">
  <img src="https://img.shields.io/badge/Object_Detection-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white" alt="Object Detection">
  <img src="https://img.shields.io/badge/Semantic_Segmentation-3776AB?style=for-the-badge" alt="Semantic Segmentation">
</p>

### Robotics, Systems and Development

<p>
  <img src="https://skillicons.dev/icons?i=linux,git,docker,kubernetes,cmake,aws" alt="Development tools">
  <img src="https://img.shields.io/badge/ROS-22314E?style=for-the-badge&logo=ros&logoColor=white" alt="ROS">
  <img src="https://img.shields.io/badge/Gazebo-F58113?style=for-the-badge&logo=gazebo&logoColor=white" alt="Gazebo">
  <img src="https://img.shields.io/badge/CUDA-76B900?style=for-the-badge&logo=nvidia&logoColor=white" alt="CUDA">
  <img src="https://img.shields.io/badge/ONNX_Runtime-005CED?style=for-the-badge&logo=onnx&logoColor=white" alt="ONNX Runtime">
</p>

---

## 🚀 Featured Projects

### 🤖 Robotic Reliability Intelligence

Machine-learning and analytics workflows for analyzing large-scale robotic telemetry, detecting operational anomalies, identifying spatial hotspots and supporting reliability investigations.

**Technologies:** Python, SQL, Scikit-learn, clustering, anomaly detection, spatial analytics and time-series analysis

---

### 🦾 Multi-Robot Embodied Intelligence

A multi-robot simulation framework for navigation, coverage, coordination and perception-driven decision-making in dynamic environments.

**Technologies:** Python, ROS, Gazebo, PyTorch, RGB/depth processing and multi-agent systems

---

### 🕸️ SITRUS

A simulation and analytical framework for sensor deployment in irregular environments, focused on coverage, connectivity, energy efficiency and spatial optimization.

**Technologies:** Python, Contiki-NG, Cooja, optimization and wireless sensor networks

---

### 🤖 LLM-Based Information Retrieval and Question Answering

A modular agent framework integrating retrieval, reasoning, tool invocation and memory for multi-step question answering and troubleshooting.

**Technologies:** Python, Hugging Face, BM25, transformers, RAG, prompt engineering and AI agents

---

## 🐍 My Contribution Journey

<div align="center">

<picture>
  <source
    media="(prefers-color-scheme: dark)"
    srcset="https://raw.githubusercontent.com/Duck-exe/Duck-exe/output/github-contribution-grid-snake-dark.svg"
  >
  <source
    media="(prefers-color-scheme: light)"
    srcset="https://raw.githubusercontent.com/Duck-exe/Duck-exe/output/github-contribution-grid-snake.svg"
  >
  <img
    alt="GitHub contribution snake animation"
    src="https://raw.githubusercontent.com/Duck-exe/Duck-exe/output/github-contribution-grid-snake.svg"
  >
</picture>

</div>

---

## 📖 Research and Publications

My research focuses on:

* Connected k-coverage in wireless sensor networks
* Irregular hexagonal tessellation and sensor placement
* Secure communication protocols
* Multi-agent spatial coordination
* Optimization and network simulation
* AI-enabled networking and robotic systems

### Selected Publications

- 📄 [**Secure Connected *k*-Coverage in Spatial Wireless Sensor Networks**](https://ieeexplore.ieee.org/abstract/document/11532813)  
  *IEEE MENACOMM, 2026*

- 📄 [**SITRUS: Simulation and Analysis of Irregular Tessellated Wireless Sensor Networks Using Cooja**](https://ieeexplore.ieee.org/abstract/document/11159227)  
  *IEEE Access, 2025*

- 📄 [**Irregular Honeycomb Network: Revolutionizing *k*-Coverage in Spatial Wireless Sensor Networks**](https://ieeexplore.ieee.org/abstract/document/10621539)  
  *IEEE DCOSS-IoT, 2024*

- 📄 [**HexTileConnect: Achieving Connected *k*-Coverage in Planar Wireless Sensor Networks Using Irregular Hexagonal Tessellation**](https://ieeexplore.ieee.org/abstract/document/10776724)  
  *IEEE ICAIGE, 2024*

<br>

- 📄 **Secure Connected *k*-Coverage in Spatial Wireless Sensor Networks**
- 📄 **SITRUS: Simulation and Analysis of Irregular Tessellated Wireless Sensor Networks Using Cooja**
- 📄 **Irregular Honeycomb Network: Revolutionizing *k*-Coverage in Spatial Wireless Sensor Networks**
- 📄 **HexTileConnect: Achieving Connected *k*-Coverage in Planar Wireless Sensor Networks Using Irregular Hexagonal Tessellation**
---

## 🚀 Current Focus

<div align="center">

🤖 AI Agents • 🧠 Large Language Models • 📷 Computer Vision

⚡ Robotics • 📊 Data Science • 🌐 Distributed Systems

</div>

---

## 🌟 Beyond Technology

When I am away from research and coding, you can find me:

* 🏊 Swimming
* 🏸 Playing Badminton
* 🚴 Cycling
* 📖 Exploring emerging AI and Robotics research

---

## 📫 Let's Connect

I am interested in opportunities and collaborations involving:

**Machine Learning • Data Science • Generative AI • AI Agents • Robotics • Computer Vision • Applied Research**

<div align="center">

<a href="mailto:maddaladakshanya@gmail.com">
  <img src="https://img.shields.io/badge/Outlook-Contact_Me-0078D4?style=for-the-badge&logo=microsoftoutlook&logoColor=white">
</a>

<a href="https://www.linkedin.com/in/dakshanya-maddala/">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>

<br><br>

### “Transforming data into intelligence and intelligence into real-world impact.”

</div>
